...
from summs_utils_v3 import gen_mult_seq_example #, gen_mult_seq_from_topology_example
import pickle
import numpy as np
...
def train_epoch(model, training_data, optimizer, opt):
    ...
    for batch in tqdm(training_data, mininterval=2,
        ...
        if opt.sle == 0 and opt.excites == 1: # we need penalize effect or non excites if nonexcites produces large probability for every instance in a sequence
          loss_cp =  F.relu((event_type!=opt.type_z)[:,:-2]* (1-(1-y_score[:,1:-1])*(1-y_score[:,2:])) /(1.0-z_score[:,:-2])- (event_type==opt.type_z)[:,:-2] * (1-(1-y_score[:,1:-1])*(1-y_score[:,2:]))/z_score[:,:-2]) 
          loss_cp = torch.sum(loss_cp)
        elif opt.sle == 0 and opt.excites == 0:
          loss_cp =  F.relu( (event_type==opt.type_z)[:,:-2] * (1-(1-y_score[:,1:-1])*(1-y_score[:,2:]))/z_score[:,:-2] - (event_type!=opt.type_z)[:,:-2]* (1-(1-y_score[:,1:-1])*(1-y_score[:,2:]))/(1.0-z_score[:,:-2]) ) 
          loss_cp = torch.sum(loss_cp)
        elif opt.sle == 1 and opt.excites == 1:
          loss_cp =  torch.mean((event_type!=opt.type_z)[:,:-2] * (1.0 - (1-y_score[:,1:-1])*(1-y_score[:,2:]))/(1.0-z_score[:,:-2])- (event_type==opt.type_z)[:,:-2] * (1-(1-y_score[:,1:-1])*(1-y_score[:,2:]))/z_score[:,:-2], dim=-1) 
          loss_cp = torch.sum(F.relu(loss_cp))
        else:
          loss_cp =  torch.mean( (event_type==opt.type_z)[:,:-2] * (1-(1-y_score[:,1:-1])*(1-y_score[:,2:]))/z_score[:,:-2]- (event_type!=opt.type_z)[:,:-2]* (1-(1-y_score[:,1:-1])*(1-y_score[:,2:]))/(1.0-z_score[:,:-2]), dim=-1) 
          loss_cp = torch.sum(F.relu(loss_cp))
...
for seed_i in range(0,5):
    ...
    parsed_args.type_z = 3 # (B,A) B excites A             (C, B) , C excites B
